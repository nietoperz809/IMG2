...
public class AnimatedGIFReader {
	...
	private byte[] decodeLZW(InputStream is) throws IOException {
	...
	private byte[] decodeLZWInterLaced(InputStream is) throws IOException	{
		int index;
		int index2 = 0;
...
//	}
	 ...
	 * @throws Exception
	 */
	protected BufferedImage getFrameAsBufferedImage(InputStream is) throws IOException {
	 ...
	 * @throws Exception
	 */
	protected BufferedImage getFrameAsBufferedImageEx(InputStream is) throws IOException {
...
//	}
	...
	private byte[] readFrame(InputStream is) throws IOException {
	...
	private void readGlobalPalette(InputStream is,int num_of_color) throws IOException {
		int index1 = 0;
	...
	private boolean readGlobalScopeData(InputStream is) throws IOException {
...
//			   backgroundColor = new Color(globalColorPalette[bgcolor]);
	...
	private byte readImageDescriptor(InputStream is) throws IOException {
		int nindex = 0;
	...
	private void readLocalPalette(InputStream is,int num_of_color) throws IOException	{
		int index1 = 0;
		int bytes2read = num_of_color*3;
		byte[] brgb = new byte[bytes2read];
		IOUtils.readFully(is,brgb,0,bytes2read);
